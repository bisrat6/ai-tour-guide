# Developer 3 ΓÇö Integrations & Monetization
...
**Parent spec:** `backend-implementation-plan.md` (v2). This document is a *track plan*, not a replacement ΓÇö where v2 specifies something (error envelope, provider adapters, testing approach), this document follows it and points at the section rather than restating it. Where this document adds something v2 does not have, that addition is flagged explicitly in ┬º1.2.

**Payment rail:** **Chapa**, not Telebirr. Rationale and the concrete differences that follow from the swap are in ┬º6.1.
...
| Area | Routes | Spec |
|---|---|---|
| Ticket validation | `POST /tickets/validate`, `POST /stub-ticket-vendor` | v2 ┬º15, extended here in ┬º10 |
| Subscription billing | `POST /admin/billing/checkout`, `GET /admin/billing/status`, `GET /admin/billing/plans`, `GET /admin/billing/payments/:txRef`, `POST /admin/billing/tier` | ┬º9 |
| Payment confirmation | `applyPaidPayment`, `scripts/reconcile-payments.ts` | ┬º6.4, ┬º6.5 |
| Tier enforcement | `requireWithinTierLimit` middleware | ┬º8 |
...
| # | v2 says | Amendment |
|---|---|---|
| A1 | ┬º20 Out of scope: "Billing or subscriptions for museums." | Removed. Superseded by ┬º┬º4ΓÇô9 of this document. What stays out of scope is any rail other than Chapa ΓÇö no cards, no bank transfer, no second mobile-money provider ΓÇö and any per-visitor charging. |
| A2 | ┬º6.1 schema has no tier, subscription, or payment models. | Additions in ┬º4. These are handed to Dev 1 as a schema patch, not applied independently ΓÇö see ┬º2.2. |
| A3 | ┬º12 defines `LlmProvider`, `TtsProvider`, `StorageProvider`. | Two more adapters, `PaymentProvider` and `TicketProvider`, following the same interface-plus-fake pattern and the same ┬º12.4 resilience wrapper. See ┬º11. |

### 1.3 What this track deliberately does not build

- **No proration, no refunds, no invoices as documents.** An upgrade mid-period extends the period (┬º7.3); it does not compute a partial credit. Refunds are handled manually in the Chapa dashboard and reconciled by a system admin via `POST /admin/billing/tier`.
- **No automatic recurring charge.** Chapa's standard checkout is a one-off hosted payment; there is no card-on-file to re-charge for a mobile-money rail. Renewal is a *reminder plus a fresh checkout*, not a silent debit. See ┬º7.2.
- **No self-serve signup.** A museum still comes into existence through `POST /admin/museums` by a system admin (v2 ┬º14.1). Billing attaches to an existing museum; it does not create one.
- **No real ticketing vendor.** Stub only, per v2 ┬º15.4, until one is chosen.
- **No tier gating for features that do not exist.** The tier table markets "beacons", "3D model hosting", "OpenHPS spatial routing", and "multi-building navigation". The schema has `Museum -> Room -> Item` and QR codes ΓÇö none of those features exist. This track builds the *mechanism* (the tier field, the limit check, the payment flow) using **room count as the enforceable proxy for "floor"**. When those features land later they plug into `TIER_LIMITS` (┬º8.1) rather than needing a new gating system. Do not write dead code for unbuilt features.
...
`requireWithinTierLimit` (┬º8) resolves a museum from the resource being acted on, which is the same resolution `requireMuseumScope` does (v2 ┬º8.4). It must not duplicate that logic with its own body-trusting version ΓÇö that would reintroduce exactly the bypass v2 ┬º8.4 exists to prevent.

**Sequencing:** build `requireWithinTierLimit` against Dev 1's day-one **stub** middleware, but treat it as unverified until Dev 1's real `requireAuth` / `requireMuseumScope` land at the end of Phase 3. The joint checkpoint in ┬º14 is where it gets re-verified against the real thing.

Everything else in this track ΓÇö the Chapa adapter, the stub ticket vendor, the schemas, the fake payment provider, the reconciler ΓÇö depends only on the finalized Prisma schema, which is fixed on day one.
...
Instead: hand Dev 1 the model definitions in ┬º4 on day one, as a patch to `prisma/schema.prisma`, to be included in the **initial** migration. The billing tables being empty in Phase 1 costs nothing; a divergent migration history costs a day.
...
- Zod schemas for every request and response in ┬º9 and ┬º10.
...
Owned outright ΓÇö no other developer should be editing these, so merge conflicts stay rare:
...
Shared ΓÇö every edit here needs a heads-up in the team channel because all three developers touch them:

| File | Dev 3's edit |
|---|---|
| `prisma/schema.prisma` | ┬º4 models, via Dev 1 (┬º2.2) |
| `prisma/seed.ts` | `TierPricing` rows |
| `src/app.ts` | Mounting the billing and tickets routers |
| `src/config/env.ts` | The variables in ┬º5 |
| `src/lib/errors.ts` | The codes in ┬º12 |
...
Handed to Dev 1 per ┬º2.2. Follows v2 ┬º6.1 conventions: explicit referential actions, `createdAt`/`updatedAt` on anything editable, indexes on every foreign key used in a filter.
...
model Museum {
  ...
  @@index([subscriptionStatus, subscriptionRenewsAt])   // drives the daily lapse sweep (┬º7.2)
}
...
model Payment {
  ...
  @@index([status, createdAt])   // drives the reconciler (┬º6.5)
}

// Audit rows attributed to no human ΓÇö the reconciler applying a payment ΓÇö
...
Added to `src/config/env.ts`, Zod-validated, process refuses to boot if malformed (v2 ┬º5).
...
ENABLE_STUB_TICKET_VENDOR=false   # must be false in production (┬º10.4)
...
**This track does not implement webhooks.** Chapa can push `charge.success` to a public endpoint, but that requires a publicly reachable URL, a tunnel in development, and a signature-verification path that is the single most error-prone part of the integration. Since the verify call is authoritative regardless of what prompted it, the webhook is only ever a latency optimisation. We drop it and prompt the verify from two places instead: the return page poll and the scheduled reconciler (┬º6.4).
...
The return-page poll is the primary confirmation path, not a fallback. The reconciler (┬º6.5) covers the case where the admin never lands back on the return page.

### 6.3 `POST /transaction/initialize` ΓÇö request construction
...
- **`amount` is a string with two decimals**, taken from `TierPricing.amountEtb`. Never accept an amount from the request body ΓÇö the client sends a `tier`, and the price is looked up server-side. A client-supplied price is a client-chosen price.
...
- A non-`success` `status` in the response body, or any non-2xx, maps to `502 UPSTREAM_FAILURE` per v2 ┬º7.2. The raw Chapa body is logged with the request ID, never returned to the client. The `Payment` row is marked `FAILED` with `failureReason` set, so a support query can see the attempt happened.

### 6.4 `applyPaidPayment` ΓÇö the single entitlement path
...
| `scripts/reconcile-payments.ts` | Scheduled, every 10ΓÇô15 minutes | The admin closed the tab, lost connectivity, or paid from another device |
...
With no webhook, the reconciler is not a safety net bolted on for completeness ΓÇö it is one of only two confirmation paths, and the only one that works when the admin's browser is closed. **Ship it scheduled, or payments will be silently lost.**

`scripts/reconcile-payments.ts`, run every 10ΓÇô15 minutes as a Render cron job:
...
- Every `PENDING` payment past 24 hours: one final verify, then `EXPIRED` if Chapa still reports nothing. The last check matters ΓÇö a payment that succeeded while both the poll and the operator were asleep gets recovered instead of written off.
- `--sweep`: flip `ACTIVE` museums past `subscriptionRenewsAt` to `PAST_DUE` (┬º7.2).
...
- The museum renews by running `POST /admin/billing/checkout` again with their current tier. `applyPaidPayment` extends the period per ┬º7.3 and flips status back to `ACTIVE`.
...
Taking the max means an early renewal *extends* rather than *truncates* ΓÇö a museum that renews a week early does not lose that week. A lapsed museum renewing after expiry starts fresh from now, which is the correct answer for a rail with no proration.
...
The split is deliberate. A **price** change is a business decision that should be a data edit, not a deploy ΓÇö hence `TierPricing`. A **limit** change is a behaviour change that needs a code review and a test run, because it alters what the API permits.

`maxRooms` is the "floor" proxy from ┬º1.3. `maxItemsPerRoom` and `maxAdminUsers` are included because they are cheap to enforce with the same middleware and they stop the obvious workaround of cramming an entire museum into one enormous room.

### 8.2 Middleware behaviour

Applies to `POST /admin/rooms`, `POST /admin/items`, and `POST /admin/museums/:id/admins`.

1. Resolve the museum **from the database record being acted on**, exactly as v2 ┬º8.4 requires ΓÇö for a room create, from `req.admin.museumId` (or the system admin's explicit `museumId`, validated to exist); for an item create, from `room.museumId` looked up via `body.roomId`. **Never from a `museumId` in the body.** This resolution is shared with `requireMuseumScope`; extract it into one helper rather than writing it twice.
...
**Downgrades never delete content.** A museum that drops from PRO to BASIC with three rooms keeps all three. They simply cannot create a fourth. Enforcement is on the *create* path only ΓÇö never write a job that prunes rooms to fit a plan.
...
All routes require `requireAuth` (v2 ┬º8.2). Scoped routes additionally resolve the museum from the record and reject cross-tenant access with `403 CROSS_TENANT_ACCESS`.
...
`null` means unlimited ΓÇö `Infinity` is not valid JSON, and serializing it produces `null` anyway, so make that intentional.

Seed prices are placeholders pending Q1 in ┬º16.
...
3. Reject with `429 RATE_LIMITED` if there are more than 5 `PENDING` payments for this museum in the last hour ΓÇö a stuck admin clicking repeatedly should not generate twenty Chapa sessions.
...
`SYSTEM_ADMIN` may pass `?museumId=`; a `MUSEUM_ADMIN`'s own museum is used and any supplied `museumId` is **ignored, not honoured** ΓÇö the same rule as v2 ┬º14.2 for rooms.
...
Returns the `Payment` projection from ┬º9.3 plus the museum's resulting `tier` and `subscriptionRenewsAt`.
...
Follows v2 ┬º15. Everything below is either that spec restated as build steps, or an addition marked as such.

### 10.1 `POST /tickets/validate`

Public ΓÇö a visitor has no account and no token. **Request:** `{ museumId, ticketCode }`. **Response:** `{ valid, ticketRequired }`.

1. Look up the museum. `404` if missing or `SUSPENDED`.
2. `ticketValidationUrl == null` ΓåÆ return `{ valid: true, ticketRequired: false }` immediately. No external call, no gate, no cost. This is the path almost every museum takes and it must add zero latency.
3. Otherwise call the URL server-side through the `TicketProvider` adapter, 5s timeout with one retry (v2 ┬º12.4), and map the response to `{ valid: boolean, ticketRequired: true }`.
...
- **Log every validation attempt** with museum ID, outcome, and a hash of the code ΓÇö never the code itself.

### 10.2 SSRF ΓÇö the part v2 does not cover

`ticketValidationUrl` is **admin-supplied input that the server makes an HTTP request to**. That is a server-side request forgery primitive by construction. A museum admin ΓÇö the least-privileged authenticated role in the system ΓÇö can point it at anything the backend container can reach: the cloud metadata endpoint at `169.254.169.254`, the internal Postgres port, another service on the private network.

`src/lib/ssrfGuard.ts`, applied both when the URL is **saved** (`PATCH /admin/museums/:id`, which is Dev 1's route ΓÇö coordinate) and again when it is **called** (DNS can be re-pointed after validation):
...
- `redirect: 'manual'` ΓÇö a `302` to `http://169.254.169.254/` defeats every check above if redirects are followed.
...
- **Registered only when `ENABLE_STUB_TICKET_VENDOR === true`**, which ┬º5 forbids in production. Not "returns 403 in production" ΓÇö the route is not mounted at all, so there is nothing to find.
...
Same pattern as v2 ┬º12: a narrow interface, a real implementation, a fake for tests, and the shared timeout/retry/circuit-breaker wrapper from ┬º12.4. Route and service code never imports a vendor SDK, and never calls `fetch` directly.
...
`fake.ts` for each drives the test suite: a scriptable fake payment provider that can be told to succeed, fail, time out, or return a mismatched amount, and a fake ticket provider with the same range. **No test ever makes a network call or spends money** ΓÇö the same rule v2 ┬º17.1 sets for the other providers.

Resilience settings for this track: Chapa 10s timeout, one retry on timeout or 5xx only; ticket vendor 5s, one retry. **Never retry the verify call more than once** ΓÇö a slow verify is better handled by the reconciler picking it up 15 minutes later than by holding an admin's polling request open.

---

## 12. Error codes owned by this track

Added to `src/lib/errors.ts`, extending v2 ┬º7.2. Same envelope, no exceptions.
...
| 422 | `TICKET_URL_INVALID` | `ticketValidationUrl` failed the SSRF guard (┬º10.2). |
...
Integration tests, Vitest and Supertest against a throwaway Postgres, fakes for both providers ΓÇö v2 ┬º17.1.

### 13.1 Tier isolation matrix

The billing extension of v2's ┬º17.2 table. Same principle: automated tests running on every commit, not a checklist run once.

| # | Scenario | Expected |
|---|---|---|
| 1 | Museum A admin calls `POST /admin/billing/checkout` with `museumId` = B | `403 CROSS_TENANT_ACCESS` |
| 2 | Museum A admin calls `GET /admin/billing/status?museumId=<B>` | `200`, containing **only A's** data ΓÇö assert the contents, not just the status |
...
| 5 | `SYSTEM_ADMIN` reads any museum's billing status | `200` ΓÇö intentional |
...
| 10 | `PAST_DUE` museum's rooms via `GET /waypoint/:id` | `200` ΓÇö visitors are never affected (┬º7.1) |
| 11 | `PAST_DUE` museum `PATCH`es an existing room | `200` ΓÇö edits are not gated (┬º8.3) |
| 12 | PRO museum downgraded to BASIC with 3 existing rooms | Rooms intact; a fourth is rejected |
| 13 | `SYSTEM_ADMIN` creates a fourth room on a PRO museum | `403 TIER_LIMIT_EXCEEDED` ΓÇö limits bind the museum, not the caller (┬º8.3) |
...
### 13.4 Chapa sandbox pass ΓÇö manual, once per environment
...
1. `POST /admin/billing/checkout` ΓåÆ open the returned `checkoutUrl` in a browser.
...
Ordered by dependency. Steps 1ΓÇô4 need nothing from Dev 1 beyond the day-one schema.

**Step 1 ΓÇö Contract (day 1, parallel with Dev 1's schema).** Schema patch (┬º4) handed over. Zod schemas and OpenAPI entries for every route in ┬º9 and ┬º10. Env additions with the production guards from ┬º5. Error codes from ┬º12. Fake providers. Fixtures for the admin app.

**Step 2 ΓÇö Ticket validation (v2 Phase 8, pullable earlier).** `TicketProvider`, the SSRF guard, `/tickets/validate`, the stub vendor, and ┬º13.3's tests. Small, self-contained, no dependency on billing ΓÇö a good first shipped increment.

**Step 3 ΓÇö Chapa adapter and the entitlement path.** `PaymentProvider` implementation against the sandbox. `applyPaidPayment` as the single shared entitlement path, with the conditional-update guard. The reconciler. ┬º13.2's tests against the fake.

**Step 4 ΓÇö Billing API.** Checkout, status, plans, payment lookup, manual tier override. `TierPricing` seed data. Audit logging on every write.

**Step 5 ΓÇö Tier enforcement.** `requireWithinTierLimit` against Dev 1's **real** middleware, not the stub. Sequence this after Dev 1's Phase 3 lands. ┬º13.1's matrix green.

**Step 6 ΓÇö Reconciliation and lifecycle.** The reconciler script, the daily lapse sweep, Render cron wiring, concurrency test (┬º13.2 #11).

**Step 7 ΓÇö Sandbox pass and hardening.** ┬º13.4 end to end. Confirm every Chapa failure mode degrades visibly, per v2 ┬º7.3.

**Joint checkpoints** ΓÇö these need all three developers, not a solo sign-off:

- **After Dev 1's Phase 4.** Re-run ┬º13.1 against the real auth middleware. The specific risk is `requireWithinTierLimit` and `requireMuseumScope` resolving the museum differently; they must share one helper.
- **After v2's Phase 9.** Full-system pass: v2 ┬º17.2's matrix plus ┬º13.1's, Dev 2's ┬º10.5 adversarial chat pass, and a full Chapa sandbox payment from checkout through verify to tier upgrade.

---

## 15. Definition of done

- [ ] Every route in ┬º9 and ┬º10 implemented, matching `openapi.yaml` exactly.
- [ ] ┬º13.1, ┬º13.2, and ┬º13.3 green in CI.
- [ ] ┬º13.4's sandbox pass completed and recorded, including the closed-tab run.
...
| # | Question | Blocks |
|---|---|---|
| Q1 | Actual ETB prices per tier. ┬º9.1's numbers are placeholders. Also: monthly or annual billing period, given that Ethiopian institutional budgets are typically annual? | Step 4 |
...
| ~~Q3~~ | ~~Does `CHAPA_WEBHOOK_SECRET` mean the dashboard's secret hash or the API secret key?~~ Retired ΓÇö there is no webhook. | ΓÇö |
| Q4 | Encrypted per-museum `ticketValidationSecret` now, or defer until a real vendor is chosen (┬º10.3)? Needs Dev 1's agreement ΓÇö it is a `Museum` field. | Step 2 |
| Q5 | Renewal reminders by email ΓÇö in scope, and if so which sender? Currently only surfaced in `GET /admin/billing/status`. | Step 6 |
