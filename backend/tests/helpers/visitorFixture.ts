/**
 * Shared fixture for the visitor-facing suites (waypoints, chat, narrate).
 *
 * Ported from dev2's tests/helpers/fixtures.ts. The prose matters here in a way
 * it does not for the admin suites: the chat tests assert that answers are
 * grounded in this content, and the offline LLM fallback answers by quoting it
 * back, so the item names and detail text are what make those assertions
 * meaningful.
 */
import { prisma } from '../../src/lib/prisma.js';

export async function seedVisitorFixture() {
  const museum = await prisma.museum.create({
    data: { name: 'Adwa Test Museum', slug: 'adwa-test', status: 'ACTIVE' },
  });
  ...
  // Suspended content must be indistinguishable from content that never
  // existed, which needs a real suspended museum to assert against.
  const suspendedMuseum = await prisma.museum.create({
    data: { name: 'Suspended Museum', slug: 'suspended-test', status: 'SUSPENDED' },
  });
