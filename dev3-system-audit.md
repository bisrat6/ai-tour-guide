# Dev 3 System Audit ΓÇö Integrations & Monetization

**Branch:** `Integrations-and-Monetization`
**Scope of this document:** the Dev 3 track only ΓÇö billing, payments, ticket validation, and subscription tier enforcement. Dev 1 and Dev 2 code is referenced only where their branches must interoperate with this one.
...
| `src/modules/dev/router.ts` ΓåÆ `POST /dev/login` | Dev 1 | Mints a token for any seeded admin by email, no password. Stands in for `POST /admin/login`. |
...
| POST | `/tickets/validate` | **none ΓÇö public** | A visitor has no account. Rate limited 10/IP/minute and 30/museum/minute. |
...
**`TierPricing`** ΓÇö keyed by tier. Holds `amountEtb` as `Decimal(10,2)`, `periodDays`, display strings, and an `active` flag.

**`Payment`** ΓÇö the billing record. Unique `txRef`, `status` in `PENDING | PAID | FAILED | EXPIRED`, plus `periodStart` and `periodEnd` so the period each payment purchased is stored rather than recomputed. `failureReason` retains why a failure happened for support queries. `onDelete: Restrict` against `Museum`, because deleting a museum must not silently erase its financial history.
...
`null` means unlimited and **must serialize as `null`, never `Infinity`** ΓÇö `Infinity` is not valid JSON and emits a bare token that breaks strict parsers. There is a test asserting the response body contains no `Infinity`.
...
- A `PAST_DUE` museum still serving `GET /waypoint/:id` ΓÇö needs **Dev 2's** visitor routes.
- A `PAST_DUE` museum still accepting a room `PATCH` ΓÇö needs **Dev 1's** admin update route.
...
| Reconciler only smoke-run against an empty payment table. | Its real branches ΓÇö mid-window recovery, 24-hour expiry, sweep ΓÇö are covered by tests using the fake provider, but never against Chapa. |
...
### D1 ΓÇö Migration SQL is excluded from version control ΓÇö **blocker**
...
### D2 ΓÇö The `lint` script cannot run ΓÇö **blocker for CI**
...
### D3 ΓÇö A provider failure at checkout is reported as `500`, not `502` ΓÇö **correctness**

In `createCheckout`, a single `try` block wraps the Chapa call *and* the two database writes that follow it. The `catch` maps only `UpstreamUnavailableError` ΓåÆ `503` and `UpstreamFailureError` ΓåÆ `502`, and rethrows everything else, which the terminal handler renders as `500 INTERNAL_ERROR`.
...
### D4 ΓÇö `tests/integration/billing-api.test.ts` is a 0-byte file ΓÇö **breaks the test run**
...
**Fix:** either restore its contents or delete the file. Note the same 0-byte symptom affected another file earlier in this project ΓÇö likely an unsaved editor buffer or a OneDrive placeholder ΓÇö so it is worth confirming that files are actually written to disk before committing.

### D5 ΓÇö No validation of the Chapa key's shape ΓÇö **operational**
...
### D6 ΓÇö Tests hardcode the database hostname
...
**Consequence:** contributors and CI runners that use a service container on a published port cannot run the tests. Minor, but it constrains how CI can be configured ΓÇö worth settling before three branches converge on one pipeline.
...
3. **Attach `requireWithinTierLimit` to the real create routes.** `POST /admin/rooms`, the item create route, and the admin-user create route. It is a no-op unless it is mounted, and this is the single easiest thing to lose in a merge ΓÇö the tier caps become decorative the moment the dev route is deleted without the middleware being reattached.
...
1. **Accept `AdminAuditLog.adminUserId` as nullable** and treat `null` as a system actor, or bring an alternative that lets the reconciler write an audit row with no human behind it. See section 4.4 ΓÇö this is a real conflict requiring a decision.
...
- [ ] **D1** ΓÇö un-ignore and commit the three `migration.sql` files. Without this nothing else can be verified on another machine.
- [ ] **D4** ΓÇö restore or delete the 0-byte `billing-api.test.ts`.
...
- [ ] **D2** ΓÇö settle the lint toolchain across all three branches, or drop the script.
- [ ] **D3** and **D5** ΓÇö apply the checkout error-mapping fix and the Chapa key guard. Both are small and both remove a class of misleading failure.
...
Seeded tenants: `adwa` (BASIC, ACTIVE, 1 room ΓÇö at cap) and `louvre` (PRO, ACTIVE, 3 rooms ΓÇö at cap). Admins are `system@adwa.local`, `admin@adwa.local`, `admin@louvre.local`.
...
| Q6 | Policy for a museum that never renews ΓÇö indefinite `PAST_DUE`, or suspension after N days? | Reconciler sweep behaviour |

Q3, which Chapa secret verifies a webhook signature, is retired ΓÇö there is no webhook. Remaining numbering matches dev3 ┬º16 so the two documents stay cross-referenceable.
