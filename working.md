...
**Payments:** Chapa sandbox. **No webhooks and no tunnel** — see "Payment confirmation without webhooks" below.
...
| 9 | Tests | done — 47 passed, 2 skipped (Dev 1/2 routes) |
...
**The reconciler is not optional.** The lazy verify only runs while an admin has the return page open. If they close the tab mid-payment, the reconciler is the sole remaining path — without it scheduled, that payment sits `PENDING` forever and the museum never gets the tier it paid for.

Schedule it every 10–15 minutes in any environment taking real money.

---

## Step 0 — Scaffold and Docker — done
...
## Step 1 — Core libs and the error envelope — done
...
## Step 2 — Schema, migration, seed — done
...
**`AdminAuditLog.adminUserId` is nullable**, with `onDelete: SetNull`. Null means the system acted with no human behind it — the reconciler applying a payment. It was originally required with `onDelete: Cascade`, which was wrong twice over: the reconciler had no actor to name, and an audit trail should outlive the account that produced it.

---

## Step 3 — Auth stubs shaped to Dev 1's contract — code done
...
- [ ] The same route returns `200` with a valid token. **Not yet confirmed** — see the note below.
...
## Step 4 — Provider adapters — code done
...
- [ ] A call to Chapa sandbox `initialize` prints a real `checkout_url`. **Blocked** — `.env` still holds the placeholder key and `PAYMENTS_PROVIDER=fake`.
...
## Step 5 — Ticket validation — code done
...
- [ ] Setting `ticketValidationUrl` to a private address is rejected with `422 TICKET_URL_INVALID`. **Cannot be tested yet** — the save-time path needs Dev 1's admin museum-update route, which does not exist. The guard itself is verified and runs on every outbound call.

Upstream failure is `502`. **Never fail open** — a gate that passes when the vendor is down is decorative.

---

## Step 6 — Chapa checkout — code done, unverified
...
## Step 8 — Billing reads, tier limits, reconciler — done

Routes in `src/modules/billing/router.ts`:

- `GET /admin/billing/plans` — `TierPricing` joined with `TIER_LIMITS`; unlimited serializes as `null`, not `Infinity`.
- `GET /admin/billing/status` — a museum admin's own museum only; a supplied `museumId` is **ignored, not honoured**.
- `GET /admin/billing/payments/:txRef` — the route the return page polls; runs a live verify through `applyPaidPayment` once the payment is more than 5 seconds old.
- `POST /admin/billing/tier` — system-only override with a required `reason`, audit-logged.
...
- [x] `tests/helpers/` — truncate + fixtures against `db-test`, JWT minting, fake provider reset.
